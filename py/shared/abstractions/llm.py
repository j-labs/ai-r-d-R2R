...
class ReasoningChoiceDelta(ChoiceDelta):
    """
    Almost the same as OpenAI's Choice, but adds reasoning_content.
    """
...
class StreamedChoice(OpenAIChoice):
    delta: ReasoningChoiceDelta
    """A chat completion delta generated by streamed model responses."""


class LLMChatCompletionChunk(ChatCompletionChunk):
    """
    Almost the same as OpenAI's ChatCompletionChunk, but adds reasoning_content to choices.
    """

    choices: list[StreamedChoice]
    """A list of chat completion choices with possible reasoning_content."""
...
class GenerationConfig(R2RSerializable):
    ...
    extended_thinking: bool = Field(
        ...
        description="Flag to enable extended thinking mode",
    )
    thinking_budget: Optional[int] = Field(
        default_factory=lambda: GenerationConfig._defaults["thinking_budget"],
        description=(
    ...
    reasoning_effort: Optional[str] = Field(
        default_factory=lambda: GenerationConfig._defaults["reasoning_effort"],
        description=(
        ...
        if (
            ...
            data["response_format"] = {
                ...
                "json_schema": {
                    ...
                    "schema": ensure_string_types(model_class.model_json_schema()),
                },
            }
...
def ensure_string_types(schema_obj):
    if isinstance(schema_obj, dict):
        if "type" in schema_obj and not isinstance(schema_obj["type"], str):
            schema_obj["type"] = str(schema_obj["type"])

        for key, value in schema_obj.items():
            if isinstance(value, (dict, list)):
                ensure_string_types(value)
    elif isinstance(schema_obj, list):
        for item in schema_obj:
            ensure_string_types(item)
